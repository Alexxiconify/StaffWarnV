...
internal class PermissionManager(private val logger: Logger, private val dataDirectory: Path) {
    ...
    fun checkPermissionAndAlert(player: Player, permission: String): String? {
        ...
        val server = player.world.name.lowercase()

        // Skip excluded servers
        if (server in excludedServers) {
            if (verboseLogging)
                logger.fine("Skipping alert for ${player.name} - server $server is excluded")
            return null
        }
        ...
        if (defaultGroupsHavePermission(context, permission)) {
            if (verboseLogging)
                logger.fine(
                    "Skipping alert for ${player.name} - permission $permission is in default groups"
                )
            return null
        }
        ...
        if (verboseLogging) {
            logger.fine(
                "Alert triggered for ${player.name} - permission $permission from $origin on server $server"
            )
        }
        return origin
    }
    ...
    fun getPermissionOrigin(
        ...
        return try {
            ...
            logger.warning("Error determining permission origin: ${e.message}")
            "unknown"
        }
    }
    ...
    private fun getConfig(file: String): FileConfig {
        ...
        try {
            ...
            logger.severe("Failed to load config file $file: ${e.message}")
            e.printStackTrace()
     ...
     */
    fun cleanupCache() {
        val permissionCacheSize = permissionCheckCache.size
        val originCacheSize = permissionOriginCache.size
    ...
    init {
        try {
            ...
            if (verboseLogging) {
                logger.fine("Default groups: ${defaultGroups.joinToString()}")
                logger.fine("Excluded servers: ${excludedServers.joinToString()}")
            }
        } catch (e: Exception) {
